...
exp=cbind(LUAD_exp,LUSC)
library(openxlsx)
write.xlsx(exp, "seqdata.xlsx")
...
#下载生存信息
setwd("D:/case_for_R/TCGA/TCGA-LUAD/")
setwd("TCGA-LUAD")
setwd("Survival_data")
library(tidyverse)
library(readxl)
clinical<-read_tsv("clinical.tsv",show_col_types =T)
...
##保存文件##
write.table(clinical,"clinical.txt",sep = "\t",row.names = T,col.names = NA,quote = F)
